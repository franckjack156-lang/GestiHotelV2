...
import { Search, User, Users, X, Loader2, Check } from 'lucide-react';
...
const UserItem: React.FC<UserItemProps> = ({ user, isSelected, onToggle, disabled = false }) => {
  return (
    <div
...
export const NewConversationDialog: React.FC<NewConversationDialogProps> = ({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(
        type === 'direct' ? 'Conversation créée avec succès' : 'Groupe créé avec succès'
      );
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Créez une conversation directe avec un utilisateur ou un groupe avec plusieurs membres.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedUsers.map(user => (
                  <Badge key={user.id} variant="secondary" className="gap-2 pr-1 pl-3 py-1.5">
          ...
          <div>
            <Label className="mb-2 block">
              {type === 'direct' ? 'Sélectionner un utilisateur' : 'Sélectionner les membres'}
              {type === 'group' && <span className="text-muted-foreground ml-1">(minimum 2)</span>}
            </Label>
